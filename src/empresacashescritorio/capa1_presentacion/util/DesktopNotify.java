...
public class DesktopNotify {
    ...
    static {
        ...
        for (int i = 0; i < defaultIcons.length; i++) {
            defaultIcons[i] = new ImageIcon(DesktopNotify.class.getResource("/empresacashescritorio/capa6_imagenes/" + (i + 1) + ".png")).getImage();
        }
    }
     ...
     * @param type An int that denotes a valid message type. Invalid values will
     * be treated as DEFAULT.
     */
    public static void showDesktopMessage( String message, int type) {
        showDesktopMessage("FiveCod", message, type, null, null, 0);
    }
     ...
     * @param type An int that denotes a valid message type. Invalid values will
     * be treated as DEFAULT.
     * @param action An ActionListener with the action to execute when the user
     ...
     * notification will stay on the screen. If set to 0, the user will have to
     * close it manually by clicking it.
     */
     ...
     * @param maxTimeMillis The maximum time in milliseconds that this
     * notification will stay on the screen. If set to 0, the user will have to
     * close it manually by clicking it. Such value recommended if you want to
     ...
     * @param type An int that denotes a valid message type. Invalid values will
     * be treated as DEFAULT.
     * @param icon A custom Image to use instead of the default icon assigned by
     * type.
     * @param maxTimeMillis The maximum time in milliseconds that this
     * notification will stay on the screen. If set to 0, the user will have to
     * close it manually by clicking it. Such value recommended if you want to
...
//            System.out.println(d);
        ...
        if (i == -1) {
            ...
            if (!title.isEmpty()) {
                rd.setFont(new Font("Tahoma", Font.BOLD, 14));
                for (int j = 0; j < tlts.length; j++) {
                    rd.drawString(tlts[j], 5 + ((icon == null && type == 0) ? 0 : 38), 20 + (15 * j));
                }
            }
            if (!message.isEmpty()) {
                rd.setFont(new Font("Tahoma", Font.PLAIN, 12));
...
//        }
    ...
    protected void sortMessage() {
        if (!title.isEmpty()) {
            tlts = splitLines(title, new Font("Verdana", Font.BOLD, 14));
        }
        if (!message.isEmpty()) {
            msgs = splitLines(message, new Font("Verdana", Font.PLAIN, 12));
        }
